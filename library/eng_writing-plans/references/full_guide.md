# Full Guide for eng_writing-plans
...
> [!IMPORTANT]
> **HARD DEPENDENCY:** Before defining tasks, you MUST read the architecture decisions stored in `/brain/decisions/` using the `list_dir` and `view_file` tools. The generated `task.md` must strictly comply with these decisions.
